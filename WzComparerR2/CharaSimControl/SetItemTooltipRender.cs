        ...
        private Bitmap RenderSetItem(bool specialPetSetEffectName, out int picHeight)
        {
            ...
            else
            {
                for (int i = 0; i < this.SetItem.CompleteCount; ++i)
                {
                    ...
                    TextRenderer.DrawText(g, "未穿戴", GearGraphics.EquipDetailFont2, new Point(252 - TextRenderer.MeasureText(g, "未穿戴", GearGraphics.EquipDetailFont2, new Size(int.MaxValue, int.MaxValue), TextFormatFlags.NoPadding).Width, picHeight), ((SolidBrush)GearGraphics.GrayBrush2).Color, TextFormatFlags.NoPadding);
                    picHeight += 18;
                }
            }
            ...
            foreach (KeyValuePair<int, SetItemEffect> effect in this.SetItem.Effects)
            {
                ...
                foreach (KeyValuePair<GearPropType, object> prop in props)
                {
                    ...
                    else if (prop.Key == GearPropType.bonusByTime)
                    {
                        ...
                        {
                            GearGraphics.DrawPlainText(g, $"{p.TermStart}小時後", GearGraphics.EquipDetailFont2, color, 10, 244, ref picHeight, 15);
