...
namespace WzComparerR2
{
    public partial class MainForm:Office2007RibbonForm, PluginContextProvider
    {
        ...
        public static Dictionary<string,string> SetEffectList = new Dictionary<string,string>();
        public static Dictionary<string,string> ItemEffectList = new Dictionary<string,string>();
            ...
            try
            {
                ...
                if(this.ribbonControl1.Expanded = UIStateConfig.Default.RibbonExpanded)
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        this.ribbonControl1.SelectedRibbonTabItem = this.ribbonTabItem2;
                        break;
                    case 3:
                        this.ribbonControl1.SelectedRibbonTabItem = this.ribbonTabItem3;
                        break;
                    default:
                        this.ribbonControl1.SelectFirstVisibleRibbonTab();
                        break;
                    }
                }
        ...
        private void OnSavePngFile(Frame frame)
        {
            if(frame.Png != null)
            {
                ...
                labelItemStatus.Text = "圖片儲存於: " + pngFileName;
            }
            else
            {
            ...
            using(OpenFileDialog dlg = new OpenFileDialog())
            {
                ...
                if(dlg.ShowDialog() == DialogResult.OK)
                {
                    ...
                    foreach(var Iter in GetNode("Effect/ItemEff.img").Nodes)
                        ItemEffectList.Add("0" + Iter.Text,Iter.FullPathToFile2());
                    */
        ...
        private void openWz(string wzFilePath)
        {
            ...
            try
            {
                ...
                labelItemStatus.Text = "Wz讀取成功,用時" + (Math.Round(QueryPerformance.GetLastInterval(),4) * 1000) + "ms, " + wz.img_number + "個img";
        ...
        private void openImg(string imgFileName)
        {
            ...
            try
            {
                ...
                OnWzOpened(new WzStructureEventArgs(wz)); //触发事件
                sw.Stop();
                labelItemStatus.Text = $"Img讀取成功,用時 {sw.ElapsedMilliseconds}ms";
                refreshRecentDocItems();
            }
            catch(FileNotFoundException)
            {
        ...
        private void buttonItemClose_Click(object sender,EventArgs e)
        {
            ...
            if(wz_f == null)
            {
                MessageBoxEx.Show("沒有正確選擇要關閉的wz","OK");
                return;
            }
        ...
        private void tsmi1Export_Click(object sender,EventArgs e)
        {
            ...
            if(img == null)
            {
                MessageBoxEx.Show("沒有選中一個用於導出的wz_im");
                return;
            }
            ...
            if(dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    MessageBoxEx.Show(ex.ToString(),"錯了");
                }
                finally
                {
                    if(fs != null)
        ...
        private void searchAdvTree(AdvTree advTree,int cellIndex,string searchText,bool exact,bool regex)
        {
            ...
            try
            {
                ...
                if(searchNode == null)
                    MessageBoxEx.Show("已經搜尋到末尾","OK");
            }
            catch(Exception ex)
            {
                MessageBoxEx.Show(this,ex.Message,"錯了",MessageBoxButtons.OK,MessageBoxIcon.Error);
            }
        }
        ...
        private void buttonItemSearchString_Click(object sender,EventArgs e)
        {
            ...
            if(!this.stringLinker.HasValues)
            {
                ...
                {
                    MessageBoxEx.Show("沒有初始化string鏈接，請手動指定一個Base.wz","OK");
                    return;
                }
                QueryPerformance.End();
                double ms = (Math.Round(QueryPerformance.GetLastInterval(),4) * 1000);
                labelItemStatus.Text = "初始化StringLinker成功: 耗時:" + ms + "ms";
            }
        ...
        private void buttonItemSelectStringWz_Click(object sender,EventArgs e)
        {
            ...
            {
                MessageBoxEx.Show("沒有開啟Base.wz","OK");
                return;
            }
            QueryPerformance.Start();
            ...
            {
                MessageBoxEx.Show("初始化StringLinker失敗","OK");
            }
        }
        ...
        private void tsmi2HandleUol_Click(object sender,EventArgs e)
        {
            ...
            if(uolNode == null)
            {
                labelItemStatus.Text = "沒有找到uol對應的節點...請試著載入更底層的父節點重新執行";
                return;
            }
            else
            {
        ...
        bool ShowEffectLinkForm;
        void CheckEffectLink(Wz_Node Node)
        {
            if(!ShowEffectLinkForm)
            {
                new EffectLinkForm().Show();
                ShowEffectLinkForm = true;
            }

            var ID = Node.Text;
            ID = ID.Replace(".img","");

            if(ItemEffectList.ContainsKey(ID) || SetEffectList.ContainsKey(ID))
            {
                EffectLinkForm.Instance.Show();
                EffectLinkForm.Instance.BringToFront();
            }
            else
                EffectLinkForm.Instance.Visible = false;
            // 
            if(ItemEffectList.ContainsKey(ID))
                EffectLinkForm.Instance.button1.Text = ItemEffectList[ID];
            if(SetEffectList.ContainsKey(ID))
                EffectLinkForm.Instance.button1.Text = SetEffectList[ID];
        }
        ...
        private void quickView(Node node)
        {
            ...
            if(wzf == null)
            {
                labelItemStatus.Text = "沒有查詢到節點所屬的Wz";
                return;
            }
        ...
        private void buttonItemClearExclusiveEquips_Click(object sender,EventArgs e)
        {
            ...
            labelItemStatus.Text = "無法重複穿戴的物品 " + count + "整理完畢";
        }
        ...
        private void buttonItemClearCommodities_Click(object sender,EventArgs e)
        {
            ...
            labelItemStatus.Text = "現金物品 " + count + "整理完畢";
        }
        ...
        private void btnEasyCompare_Click(object sender,EventArgs e)
        {
            if(compareThread != null)
            {
                ...
                if(DialogResult.Yes == MessageBoxEx.Show("在執行一個對比操作，要中止嗎?","Compare",MessageBoxButtons.YesNoCancel))
                {
            ...
            if(dlg.ShowDialog() == DialogResult.OK)
            {
                compareThread = new Thread(() =>
                {
                    ...
                    try
                    {
                        ...
                        while(true)
                        {
                            string txt = string.Format("待比較wz文件 :\r\n\r\n  new : {0} (V{1})\r\n  old : {2} (V{3})\r\n\r\n如果繼續對比請選擇Yes，如果想交換文件順序請選擇No。",
                    ...
                    {
                        MessageBoxEx.Show(this,"比較已經中止。","Compare");
                    }
                    catch(Exception ex)
                    {
                        MessageBoxEx.Show(this,"出現異常" + ex.ToString(),"Compare");
                    }
                    finally
                    {
                        sw.Stop();
                        compareThread = null;
                        labelXComp1.Text = "wz比對結束...時間共用" + sw.Elapsed.ToString();
                        labelXComp2.Text = "";
                    }
                });
        ...
        public static Wz_Node GetNode(string Path,bool Iss = false)
        {
            Wz_Node Node = PluginBase.PluginManager.FindWz(Path);
            if(Node != null)
            {
                if(Node.Value is Wz_Uol)
                {
                    ...
                    if(Node2.FindNodeByPath("_outlink",true) != null)
                    {
                        ...
                        {
                        case "Mob":
                            return PluginBase.PluginManager.FindWz(LinkStr);
                            break;
                        ...
                        case "Map":
                            ...
                            break;
                        case "Skill":
                            return MainForm.GetNode(LinkStr);
                            break;
                        default:
            ...
            else
            {
                ...
                var Node2 = PluginBase.PluginManager.FindWz(Split[0]);
                int Count = 0;
                string Str = "";
                string Path1 = "";
                string Path2 = "";
        ...
        void IconsButtonClick(object sender,EventArgs e)
        {
            if(IconsForm.Instance == null)
                new IconsForm().Show();
            else
                IconsForm.Instance.Show();
        }
        ...
        private void MainForm_Load(object sender,EventArgs e)
        {
            ...
            DB2Button.Click += new System.EventHandler(this.DB2ButtonClick);
            ...
            RibbonBarDB2.Text = "DB2";
            RibbonBarDB2.Items.Add(DB2Button);
            this.ribbonPanel1.Controls.Add(RibbonBarDB2);
            //
            IconsButton = new DevComponents.DotNetBar.ButtonItem();
            IconsButton.Text = "圖示預覽";
            IconsButton.FixedSize = new Size(60,65);
            IconsButton.Click += new System.EventHandler(this.IconsButtonClick);
            ...
            SearchWzButton.Text = "Wz 搜尋";
            SearchWzButton.FixedSize = new Size(54,65);
            ...
            MapObjButton.Text = "地圖物件";
            MapObjButton.FixedSize = new Size(58,65);
