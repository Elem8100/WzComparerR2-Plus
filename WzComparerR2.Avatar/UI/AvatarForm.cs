...
namespace WzComparerR2.Avatar.UI
{
    internal partial class AvatarForm:DevComponents.DotNetBar.OfficeForm
    {
        ...
        private void button1_Click(object sender,EventArgs e)
        {
            ...
            var bone = this.avatar.CreateFrame(bodyFrame,emoFrame,tamingFrame);
            var frame = this.avatar.DrawFrame(bone);
            var SavePath = System.Environment.CurrentDirectory + "\\Images\\" + GetAllPartsTag() + ".png";
            //Directory.CreateDirectory(System.Environment.CurrentDirectory+"\\Images");

            if(File.Exists(SavePath))
                return;

            frame.Bitmap.Save(SavePath,System.Drawing.Imaging.ImageFormat.Png);
            if(LoadAvatarForm.Instance!=null && LoadAvatarForm.Instance.Visible)
            {   
